...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        attn = torch.matmul(q.clone().detach() / self.temperature, k.clone().detach().transpose(2, 3))

        if mask is not None:
            attn = attn.masked_fill(mask.clone().detach() == 0, -1e9)

        attn = self.dropout(F.softmax(attn.clone().detach(), dim=-1))
        output = torch.matmul(attn.clone().detach(), v.clone().detach())
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = self.w_qs(q.clone().detach()).view(sz_b, len_q, n_head, d_k)
        k = self.w_ks(k.clone().detach()).view(sz_b, len_k, n_head, d_k)
        v = self.w_vs(v.clone().detach()).view(sz_b, len_v, n_head, d_v)

        # Transpose for attention dot product: b x n x lq x dv
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)

        if mask is not None:
            mask = mask.clone().detach().unsqueeze(1)   # For head axis broadcasting.

        q, attn = self.attention(q.clone().detach(), k.clone().detach(), v.clone().detach(), mask=mask)

        # Transpose to move the head dimension back: b x lq x n x dv
        # Combine the last two dimensions to concatenate all the heads together: b x lq x (n*dv)
        q = q.transpose(1, 2).contiguous().view(sz_b, len_q, -1)
        q = self.dropout(self.fc(q.clone().detach()))
        q += residual

        q = self.layer_norm(q.clone().detach())
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.w_2(self.relu(self.w_1(x.clone().detach())))
        x = self.dropout(x.clone().detach())
        x += residual

        x = self.layer_norm(x.clone().detach())
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, slf_attn_mask=None):
        enc_output, enc_slf_attn = self.slf_attn(
            enc_input.clone().detach(), enc_input.clone().detach(), enc_input.clone().detach(), mask=slf_attn_mask.clone().detach())
        enc_output = self.pos_ffn(enc_output.clone().detach())
        return enc_output.clone().detach(), enc_slf_attn
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask, return_attns=False):
        ...
        enc_output = src_seq.clone().detach()
        if self.scale_emb:
            enc_output *= self.d_model ** 0.5
        enc_output = self.dropout(self.position_enc(enc_output.clone().detach()))
        enc_output = self.layer_norm(enc_output.clone().detach())

        for enc_layer in self.layer_stack:
            enc_output, enc_slf_attn = enc_layer(enc_output.clone().detach(), slf_attn_mask=src_mask.clone().detach())
            enc_slf_attn_list += [enc_slf_attn.clone().detach()] if return_attns else []

        if return_attns:
            return enc_output.clone().detach(), enc_slf_attn_list
        return enc_output.clone().detach()
...
class CleanUNet(nn.Module):
    ...
    def forward(self, noisy_audio):
        noisy_audio = noisy_audio.clone().detach()
        # (B, L) -> (B, C, L)
        ...
        noisy_audio /= std.clone().detach()
        x = padding(noisy_audio.clone().detach(), self.encoder_n_layers, self.kernel_size, self.stride)
        
        # encoder
        skip_connections = []
        for downsampling_block in self.encoder:
            x = downsampling_block(x.clone().detach())
            skip_connections.append(x.clone().detach())
        ...
        x = self.tsfm_conv1(x.clone().detach())  # C 1024 -> 512
        x = x.permute(0, 2, 1).clone().detach()
        x = self.tsfm_encoder(x.clone().detach(), src_mask=attn_mask.clone().detach())
        x = x.permute(0, 2, 1).clone().detach()
        x = self.tsfm_conv2(x.clone().detach())  # C 512 -> 1024

        # decoder
        for i, upsampling_block in enumerate(self.decoder):
            skip_i = skip_connections[i].clone().detach()
            x += skip_i[:, :, :x.shape[-1]].clone().detach()
            x = upsampling_block(x.clone().detach())

        x = x[:, :, :L].clone().detach() * std.clone().detach()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str, default='configs/Valentini.json', 
